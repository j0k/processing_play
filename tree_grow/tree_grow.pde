...
void setup() 
{
  ...
  ellipseMode(CENTER);
  smooth();
  startPoint = new PVector(width/2, height/1);
...
void draw() 
{
  ...
  for(int i = 1; i < c; i ++)
  {
    land.strokeWeight(myTree.twig[(int)myTree.map[i].x].thickness[(int)myTree.map[i].y]);
    land.line(myTree.twig[(int)myTree.map[i].x].location[(int)myTree.map[i].y - 1].x, myTree.twig[(int)myTree.map[i].x].location[(int)myTree.map[i].y - 1].y,
  ...
  if (c == count){
    for(int i = 0; i < myTree.twig.length; i++)
    {
      ...
      land.ellipse(myTree.twig[i].location[num].x, myTree.twig[i].location[num].y, e1, e1);
